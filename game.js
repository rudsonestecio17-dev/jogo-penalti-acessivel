...
// --- ASSETS VETORIAIS (CENÁRIO, JOGADOR, GOLEIRO, BOLA) ---
const images = {
  ball: new Image(),
  keeper: new Image(),
  player: new Image()
...
// Jogador (Batedor de Pênalti)
images.player.src = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 160"><rect x="30" y="40" width="40" height="60" rx="8" fill="%230288d1"/><circle cx="50" cy="22" r="18" fill="%23ffcc80"/><rect x="30" y="100" width="18" height="50" fill="%23ffffff"/><rect x="52" y="100" width="18" height="50" fill="%23ffffff"/><rect x="12" y="45" width="18" height="40" fill="%230288d1"/></svg>';
...
function update() {
  if (!isShot && !isRunning) {
    ...
    // Animação da corrida do jogador até a bola
    playerX += 3;
    playerY -= 1;
    if (playerX >= 370) {
      isRunning = false;
    ...
    // Movimento da bola em direção ao gol
    if (ballY > 160) {
      ballY -= 10;
...
function draw() {
  ...
  // 1. Arquibancada / Torcida (Cenário de Fundo)
  ...
  // 2. Gramado com Listras e Linhas da Grande Área
  let fieldGradient = ctx.createLinearGradient(0, 100, 0, canvas.height);
  ...
  // 3. Trave Reforçada de Alta Visibilidade
  ...
  // 5. Jogador (Chutador)
  if (!isShot) {
    ctx.drawImage(images.player, playerX, playerY, 60, 90);
  }
  ...
  ctx.fillRect(20, 20, 170, 45);
  ctx.strokeStyle = '#ffffff';
  ctx.lineWidth = 2;
  ctx.strokeRect(20, 20, 170, 45);
...
function handleAction() {
  ...
  // Inicia animação de corrida do jogador
  isRunning = true;
  ...
  // Validação do resultado sincronizada após a corrida e viagem da bola
  setTimeout(() => {
